...
class load_leveling_main:
    ...
    def cerate_loadleveling(self,get):
        ...
        get.type = '1';
        s.SetProxy(get);
        public.WriteLog('负载均衡','添加负载均衡['+upName+']');
...
location ~ \.well-known{
    ...
    def modify_node_state(self,get):
        ...
        for i in xrange(len(data)):
            if data[i]['name'] != upName: continue;
            for n in  xrange(len(data[i]['nodes'])):
    ...
    def get_node_list(self,get):
        ...
        for i in xrange(len(levelingInfo['nodes'])):
            url = 'http://' + levelingInfo['nodes'][i]['server'] + ':' + str(levelingInfo['nodes'][i]['port']) + heartbeat['path'];
    ...
    def __write_to_conf(self,upName):
        ...
        for node in levelingInfo['nodes']:
            ...
            upNodes += '    server ' + node['server'] + ':' + str(node['port']) + ' max_fails=' + str(node['max_fails']) + ' fail_timeout=' + str(node['fail_timeout']) + 's ' +  states[node['state']]+";\n";
    ...
    def _check_node(self):
        ...
        for i in xrange(len(data)):
            for n in xrange(len(data[i]['nodes'])):
                ...
                urlstr = 'http://' + node['server'] + ':' + str(node['port']) + heartbeat['path'];
